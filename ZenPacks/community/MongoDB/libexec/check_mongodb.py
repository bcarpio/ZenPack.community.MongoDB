...
def check_memory(con, warning, critical, perf_data):
    ...
    try:
        ...
        # convert to gigs
        #
        mem_resident = float(data['mem']['resident'])
        mem_virtual = float(data['mem']['mapped']) 
        mem_mapped = float(data['mem']['virtual'])
        message = "Memory Usage: %.2fMB resident, %.2fMB mapped, %.2fMB virtual" % (mem_resident, mem_mapped, mem_virtual)
        if perf_data:
            message += " | memory_usage=%.3fMB;%.3f;%.3f" % (mem_resident, warning, critical)
            message += " memory_mapped=%.3fMB" % mem_mapped
            message += " memory_virtual=%.3fMB" % mem_virtual
